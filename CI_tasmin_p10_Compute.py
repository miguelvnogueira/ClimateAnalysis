...
for cmod in range(0,nMod):
	...
	for t in range(0,Lt):
		if Ano>=Anoi and Ano<=Anof:
			if ndias[cmod]==366 and Mes==2 and Dia==29:
				...
				Lta=len(tasmin[ti:tf,0,0])
				data[Diaj-1,(Ano-Anoi)*31:(Ano-Anoi)*31+Lta,:,:]=tasmin[ti:tf,:,:]
	...
	P10=np.zeros([ndmax,Ly,Lx])

	for diaj in range(0,ndmax):
		P10[diaj,:,:]=np.nanpercentile(data[diaj,:,:,:],10,axis=0)
	...
	# save 10th percentiles
	dirsave=driveSAVE+'/'+Cenario+'/tasmin_P10'
        if os.path.isdir(dirsave)==False:
                os.mkdir(dirsave)

	urlsave=dirsave+'/PI_tasminP10_EUR-11_'+GCMs[cmod]+'_'+Cenario+'_'+EXPs[cmod]+'_'+RCMs[cmod]+'_'+VERs[cmod]+'_day_'+str(Anoi)+str(Anof)+'.nc'
        f = Dataset(urlsave,'w',format='NETCDF4')
        f.createDimension('mm',ndmax)
        f.createDimension('yy',Ly)
        f.createDimension('xx',Lx)

       	TMmonw = f.createVariable('tasmin_p10', 'd', ('mm','yy','xx'))
        TMmonw[:,:,:]=P10[:,:,:]
        TMmonw.units='deg C'
